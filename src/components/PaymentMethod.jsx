...
function PaymentMethod({ carrito, onComprobanteSubido }) {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setPreview(url);
      onComprobanteSubido(file); // Pasamos el archivo al componente padre
    }
  };
  ...
  return (
    <div className="bg-zinc-900/50 p-8 rounded-3xl border border-purple-500/30 backdrop-blur-md shadow-[0_0_20px_rgba(168,85,247,0.1)]">
      ...
      <div className="bg-black p-6 rounded-2xl border border-zinc-800 text-center mb-8">
        <h3 className="text-cyan-400 font-bold mb-4 tracking-widest uppercase">Escanea el QR</h3>
        <img src="/yape-qr.png" alt="QR Yape" className="w-64 mx-auto rounded-xl border border-zinc-700" />
        <h3 className="text-3xl font-black text-white mt-6 tracking-[0.2em]">921 629 315</h3>
      </div>
      ...
      <div className="space-y-4">
        ...
        {preview && (
          <div className="mt-4 relative animate-in fade-in zoom-in">
            <img src={preview} alt="Preview" className="w-full h-48 object-cover rounded-xl border border-cyan-500/50" />
            <button 
              onClick={() => { setPreview(null); onComprobanteSubido(null); }}
              className="absolute top-2 right-2 bg-red-500 p-2 rounded-lg text-xs font-bold"
            >
              ELIMINAR
            </button>
          </div>
